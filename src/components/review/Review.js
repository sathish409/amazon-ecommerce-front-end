...
import { postReviewAction } from "../../pages/product/ProductAction";
...
export const Review = ({ _id, productId, productName }) => {
  ...
  const handleOnSubmit = async (e) => {
    ...
    // if status is success then call the review api and fetch all the reviews and update the store in product action file
            ...
            {Array(5)
              ...
              .map((str, i) => (
                <FaStar
                  ...
                  className={
                    rating.num > i ? "new-star text-warning" : "new-star"
                  }
                />
              ))}
          </div>
